 ...
 try{
      const { data } =await axios.get('/api/show/now-playing',{
        ...
        if(data.success){
          ...
          toast.error(data.message || 'Could not fetch movies')
        }
      
 }catch (error){
console.error('Error fetching movies:',error)
toast.error(error.response?.data?.message || 'Could not fetch movies')
 }
}, [axios, getToken]);
            ...
            {nowPlayingMovies.map((movie) =>(
              <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:translate-y-1 transition duration-300`} onClick={()=> setSelectedMovie(movie.id)}>
                    <div className='relative rounded-lg overflow-hidden'>
                      {movie.poster_path ? (
                        <img src={movie.poster_path.startsWith('http') ? movie.poster_path : image_base_url + movie.poster_path} alt="" className='w-full object-cover brightness-90' />
                      ) : (
                        <div className='w-40 h-60 bg-primary/10 flex items-center justify-center text-sm text-gray-400'>
                          No poster
                        </div>
                      )}
              ...
              {selectedMovie === movie.id && (
                ...
                <p className='font-medium truncate'>{movie.title || 'Untitled movie'}</p>
                 <p className='text-gray-400  text-sm'>{movie.release_date || 'Release date N/A'}</p>
              </div>
            ))}
        </div>
      </div>
